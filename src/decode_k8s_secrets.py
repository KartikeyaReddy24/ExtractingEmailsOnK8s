...
def decode_kubernetes_secrets():
    ...
    secret_key_decoded = base64.b64decode(secret.data["secret-access-key"]).decode(
        "utf-8"
    )
    ...
    # Decode the base64-encoded access key and secret key values
    postgres_user_decoded = base64.b64decode(secret.data["postgres-user"]).decode(
        "utf-8"
    )
    postgres_password_decoded = base64.b64decode(
    ...
    return (
        access_key_decoded,
        secret_key_decoded,
        postgres_user_decoded,
        postgres_password_decoded,
    )
